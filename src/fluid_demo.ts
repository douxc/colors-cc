...
<head>
    ...
    <style>
        body { font-family: sans-serif; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100vh; margin: 0; background: #111; color: white; }
        ...
        pre { background: #000; padding: 15px; border-radius: 8px; font-size: 0.85em; overflow-x: auto; color: #00FF41; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="controls">
            ...
            <div class="control-group" style="grid-column: span 2;">
                <label>Color Stops (Comma separated)</label>
                <input type="text" id="stops-input" value="#FCEE09, #00FF41, #00B8FF, #FF003C">
            </div>
        </div>

        <h3>Generated SVG URL:</h3>
    ...
    <script>
        ...
        function generateSVG() {
            ...
            const speed = 21 - speedRange.value; // Invert for duration
            
            // Simulation of what the API would return
            const svgContent = \`
                <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
                    <defs>
                        <linearGradient id="g" x1="0%" y1="0%" x2="100%" y2="100%">
                            \${stops.map((c, i) => \`
                                <stop offset="\${(i / (stops.length - 1)) * 100}%" stop-color="\${c.replace('%23', '#')}">
                                    <animate attributeName="stop-color" 
                                        values="\${stops.map((_, j) => stops[(i + j) % stops.length].replace('%23', '#')).join(';')}" 
            ...
            // Update URL Preview
            const apiURL = \\\`https://colors-cc.top/api/placeholder?w=800&h=400&fluid=true&stops=\\\${stops.join(',')}\\\`;
