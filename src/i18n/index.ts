...
const homeEnglishToChinese: readonly Replacement[] = [
  ...
  ['The most-used tool on colors-cc. PNG/JPEG smart compression, watermarking, and fan-layout proofs run entirely in your browser - your files never leave this device.', 'colors-cc 上使用最多的工具。PNG/JPEG 智能压缩、加水印与扇形排版，全部在你的浏览器本地完成，图片不会离开当前设备。'],
...
const imageChineseToEnglish: readonly Replacement[] = [
  ...
  ['在线图片压缩工具 · PNG/JPEG 压缩、水印与排版 | colors-cc', 'Image Compressor · PNG/JPEG & Watermark | colors-cc'],
  ['免费在线图片压缩工具，在浏览器本地压缩 PNG/JPEG、加水印并排版，全程不上传。支持 PNG-8 量化、OxiPNG 无损与 JPEG 自适应质量。', 'Compress PNG/JPEG, add watermarks, and arrange images locally in your browser - nothing is uploaded. Supports PNG-8 quantization, OxiPNG lossless, and adaptive JPEG quality.'],
  ['关于这个图片压缩工具', 'About this image compressor'],
  ['这是 colors-cc 上使用最多的工具。所有压缩、加水印与排版都在你的浏览器本地完成，图片不会上传，适合处理网页素材、确认图与隐私图片。', 'This is the most-used tool on colors-cc. All compression, watermarking, and layout run locally in your browser - images are never uploaded, making it ideal for web assets, proofs, and private photos.'],
  ...
  ['<strong>全程本地</strong>：解码、压缩、编码都在浏览器中完成，文件不会离开你的设备。', '<strong>Fully local</strong> Decoding, compression, and encoding all happen in your browser; files never leave your device.'],
